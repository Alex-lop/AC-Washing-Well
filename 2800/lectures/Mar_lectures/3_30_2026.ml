(* OK week 12 we have a Midterm next Wednesday, so lock in!!*)
 (* Week 11 *)
 (*  Compiling! *)
...
let e0 = App(Add, [Nat 1, Nat 2])


let rec eval_succedes e : bool =
    try
        let _ = execute prg in
        true
    with _ -> false
...
let compiler_correct (e : CalcLang.expr) : bool =
    eval_succuedes e -> (fun () -> execute (compile e) = conv (CalcLang.eval e))
